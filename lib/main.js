...
let visualConversationInProgress = false;
let transcriptionForVisualConversation = "";
let pictureForVisualConversation;
let translation = "";
let translation2 = "";
let translated;
let detectedText = "";

const pollyNeuralVoices = ["Vicki", "Gabrielle", "Lupe", "Lucia", "Olivia", "Amy", "Emma", "Brian", "Aria", "Ayanda", "Ivy", "Ruth", "Kendra", "Kimberly", "Salli", "Joey", "Justin", "Kevin", "Matthew", "Bianca", "Seoyeon", "Camila", "Lucia", "Lea", "Takumi", "Kajal", "Zhiyu", "Zeina", "Arlet", "Jitka", "Sofie", "Aria", "Ayanda", "Suvi", "Sabrina", "Ola", "Ines", "Elin", "Zayd"];
...
const pollyGenerativeVoices = ["Vicki", "Bianca", "Lupe", "Mia", "Lucia", "Lea", "Ruth", "Amy", "Olivia", "Kajal", "Ayanda"];
...
let streamAudioToWebSocketForVisualConversation = function (userMediaStream) {
    micStream = new mic(); //let's get the mic input from the browser, via the microphone-stream module
    micStream.on("format", function (data) {
        inputSampleRate = data.sampleRate;
    });
    ...
    wireSocketEventsForVisualConversation(); // handle messages, errors, and close events
};
...
function setLanguageVisualConversation() {
    languageCode = new URLSearchParams(window.location.search).get("language");
    sampleRate = 44100;
}
...
function wireSocketEventsForVisualConversation() {
    // handle inbound messages from Amazon Transcribe
    socket.onmessage = function (message) {
        //convert the binary event stream message to JSON
        let messageWrapper = eventStreamMarshaller.unmarshall(
        ...
        if (messageWrapper.headers[":message-type"].value === "event") {
            handleEventStreamMessageForVisualConversation(messageBody);
        } else {
            transcribeException = true;
        }
    };
...
let handleEventStreamMessageForVisualConversation = function (messageJson) {
    let results = messageJson.Transcript.Results;
    if (results.length > 0) {
        if (results[0].Alternatives.length > 0) {
            let transcript = results[0].Alternatives[0].Transcript;
            ...
            // if this transcript segment is final, add it to the overall transcription
            if (!results[0].IsPartial) {
                transcriptionForVisualConversation += transcript + "\n";
            }
        }
    }
};
...
$("#take-photo-visual-conversation").click(function () {
    document.getElementById("audioSource").src = "data:audio/mpeg;base64,";
    document.getElementById("audioPlayback").load();
    if (visualConversationInProgress == false) {
        visualConversationInProgress = true;
        pictureForVisualConversation = webcam.snap();
        document.getElementById("cameraFlip").style.display = "none";
        if (document.getElementById("translateTextTo")) 
            document.getElementById("translateTextTo").selectedIndex = 0; 
        setLanguageVisualConversation(); // set the language and region from the dropdowns
        window.navigator.mediaDevices
            .getUserMedia({
                // first we get the microphone input from the browser (as a promise)
                video: false,
            ...
            .then(streamAudioToWebSocketForVisualConversation) // we convert the mic stream to binary event stream messages when the promise resolves
            .catch(function (error) {
                console.log(error);
            });
        document.getElementById("listening-animation-visual-conversation").style.display = "flex";    
    } else {
        visualConversationInProgress = false;
        closeSocket();
        document.getElementById("listening-animation-visual-conversation").style.display = "none";
        waitForTranscriptionThenProcessImage();
    }
});
...
function waitForTranscriptionThenProcessImage() {
    if(transcriptionForVisualConversation.length !== 0){
        console.log("Question: " + transcriptionForVisualConversation);
        ProcessImage(pictureForVisualConversation);
    }
    else setTimeout(waitForTranscriptionThenProcessImage, 500);
}

//Loads selected image and unencodes image bytes for Rekognition/Textract API
function ProcessImage(picture) {

    if (window.location.href.indexOf("f-see-document") != -1) {
        ...
        reader.onload = (function (theFile) {
            return function (e) {
                ...
                var bedrockruntime = new AWS.BedrockRuntime({region: "eu-central-1"});                

                var capturedImage = e.target.result;
                var capturedImageBase64String = btoa(
                    new Uint8Array(capturedImage)
                      .reduce((data, byte) => data + String.fromCharCode(byte), '')
                  );

                var prompt = `
                You are an assistive AI helping a person with visual impairment explore their surroundings. Your task is to provide an accurate, concise, and informative description of the image attached to this message. The image was just captured by the user's smartphone camera.

                Key instructions:
                1. Describe the main elements of the scene without explicitly referring to the image.
                2. Use neutral language, such as "There is X" or "I can Y". Never start your sentences with "The images shows".
                3. Focus on relevant details that help the user understand their environment. Avoid general comments and considerations about what the overall scene conveys.
                4. Use clear, specific language without technical jargon.
                5. If uncertain about any aspect, explicitly state your uncertainty.
                6. Do not invent or assume details not present in the scene.
                7. Provide the description in the language specified by the following code: ` + languageFromURL + ` (ISO 639-3 2-digit language code, sometimes followed by ISO 3166 2-digit country code).
                8. Your response should only inlude the description. Don't add any additional content or comment to acknowledge the request. Also, don't include the language code in your answer.

                Remember that your response will be read aloud by a text-to-speech engine, so structure your response accordingly.
                `;
                            ...
                            "content": [
                                {
                                    ...
                                    "source": {
                                        ...
                                        "data": capturedImageBase64String
                                    }
                                },
                                {
                                    "type": "text",
                ...
                bedrockruntime.invokeModel(bedrockParams, function(err, data) { //  bedrockruntime.converse(bedrockParams, function(err, data) {
                    if (err) {
                        console.log(err, err.stack); // an error occurred
                        console.log("Sorry, something went wrong. Please try again!");
                        document.getElementById("show-text").style.display = "block";
                        document.getElementById("detectedText").innerHTML = "Sorry, something went wrong. Please try again!";
                        speakDetectedTextFromWebcam("Sorry, something went wrong. Please try again!", "en-US");
                    } else {
                        var responseBody = JSON.parse(String.fromCharCode.apply(null, new Uint8Array(data.body)));           // successful response
                        var sanitizedString = document.createElement('div');
                        document.getElementById("show-text").style.display = "block";

                        var decodedString = decodeURIComponent(escape(responseBody.content[0].text));
                        sanitizedString.textContent = decodedString;
                        document.getElementById("detectedText").innerHTML = sanitizedString.innerHTML;
                        $("#detectedText").css("direction", "ltr");
                        speakDetectedTextFromWebcam(decodedString, languageFromURL);
                    }
                });
            };
        })(blob);
    ...
    else if (window.location.href.indexOf("f-see-conversation-objects") != -1) {
        ...
        reader.onload = (function (theFile) {
            return function (e) {
                ...
                var prompt = `
                You are an assistive AI helping a person with visual impairment explore their surroundings. Your task is to provide an accurate, concise, and informative answer to a question based on the image attached to this message. The image was just captured by the user's smartphone camera.

                Key instructions:
                1. Answer the question and describe the main elements of the scene without explicitly referring to the image.
                2. Use neutral language, such as "There is X" or "I can Y". Never start your sentences with "The images shows".
                3. Focus on relevant details that help the user understand their environment. Avoid general comments and considerations about what the overall scene conveys.
                4. Use clear, specific language without technical jargon.
                5. If uncertain about any aspect, explicitly state your uncertainty.
                6. Do not invent or assume details not present in the scene.
                7. Provide the response in the language specified by the following code: ` + languageFromURL + ` (ISO 639-3 2-digit language code, sometimes followed by ISO 3166 2-digit country code).
                8. Your response should only inlude the answer to the question. Don't add any additional content or comment to acknowledge the request. Also, don't include the language code in your answer.
                9. The question provided by the user in the language specified above, which is the question you are supposed to answer, is the following: ` + transcriptionForVisualConversation + `
                Remember that your response will be read aloud by a text-to-speech engine, so structure your response accordingly.
                `;
                ...
                bedrockruntime.invokeModel(bedrockParams, function(err, data) { //  bedrockruntime.converse(bedrockParams, function(err, data) {
                    if (err) {
                        ...
                        document.getElementById("detectedText").innerHTML = sanitizedString.innerHTML;
                        $("#detectedText").css("direction", "ltr");
                        speakDetectedTextFromWebcam(decodedString, languageFromURL.substring(0, 2));
                    }
                    transcriptionForVisualConversation = "";
                });
            };
        })(blob);
